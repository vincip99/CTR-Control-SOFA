...
class KeyBoardController(Sofa.Core.Controller):
    ...
    def onKeypressedEvent(self, c):
        ...
        if key == "1":
            self.tube = 1
            Sofa.Helper.msg_info("KeyBoardController", f"Tube {colored_tube_number(self.tube)} selected")
            ...
            Sofa.Helper.msg_info("KeyBoardController", f"Tube {colored_tube_number(self.tube)}: shortening of {colored(self.translation_step, 'yellow')} mm")
            self.translate(self.tube, - self.translation_step)		
        elif key == "W":   
            Sofa.Helper.msg_info("KeyBoardController", f"Tube {colored_tube_number(self.tube)}: elongation of {colored(self.translation_step, 'yellow')} mm")
            self.translate(self.tube, self.translation_step)
        elif key == "A":    
            Sofa.Helper.msg_info("KeyBoardController", f"Tube {colored_tube_number(self.tube)}: clockwise rotation of {colored(np.rad2deg(self.rotation_step), 'yellow')}°")
            self.rotate(self.tube, - self.rotation_step)		
        elif key == "D":   
            Sofa.Helper.msg_info("KeyBoardController", f"Tube {colored_tube_number(self.tube)}: counter-clockwise rotation of {colored(np.rad2deg(self.rotation_step), 'yellow')}°")
            self.rotate(self.tube, self.rotation_step)
    ...
    def get_position_value(self):
        try:
            # Access the tip position (last node of the DOFs MechanicalObject)
    ...
    def get_force_value(self):
        ...
        if forces_vector is not None and forces_vector.size > 0 and (t - self.last_print_time) > self.print_interval:
            ...
            # force_value = forces_vector[0]*0.0001 # Only X-component
            Sofa.Helper.msg_info("KeyBoardController", f"Force Sensor: \033[1;93mF = {force_value:.3f} N\033[0;0m\n")
